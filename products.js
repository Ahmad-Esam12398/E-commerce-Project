...
products.forEach(productInfo => {
  ...
  hoverIcon.addEventListener('click', function () {
    const productId = productDetails.id;
    if (!cart[productId]) {
      cart[productId] = { ...productDetails, cardquantity: 1 };
...
function reloadCard() {
  ...
  for (const productId in cart) {
    ...
    newDiv.innerHTML = `
     <div> <img src="${productDetails.image}" /></div>
     <h2>${productDetails.name}</h2>
     <div class="plusevent"><span class="minus">-</span>
     <span class="num">${productDetails.cardquantity}</span>
     <span class="plus">+</span></div>
     <p>$${productDetails.price * productDetails.cardquantity}</p>
     <button class="remove-button" data-product-id="${productId}"><i class="fa-regular fa-circle-xmark"></i></button> </div>
    `;
